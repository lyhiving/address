...
class address {
    ...
    */
    public static function fuzz( $addr ) {
        ...
        if ( $street ) {
            $streetlent = mb_strlen( $street );
            $streetlen = 1-$streetlent;
            if ( mb_strpos( $street, '地区' ) || mb_strpos( $street, '区' ) || mb_strstr( $street, '镇' ) || mb_strstr( $street, '乡' ) || mb_strstr( $street, '街道' ) || mb_strstr( $street, '新城' ) || mb_strstr( $street, '（其他片区）' ) ) {
                if ( $tmp_pos = mb_strpos( $street, '（其他片区）', $streetlen ) ) {
                    $a4 = mb_substr( $street,  0, $tmp_pos+6 );
                } else if ( $tmp_pos = mb_strpos( $street, '街道', $streetlen ) ) {
                    $a4 = mb_substr( $street,  0, $tmp_pos+2 );
                } else if ( $tmp_pos = mb_strpos( $street, '新城', $streetlen ) ) {
                    $a4 = mb_substr( $street, 0, $tmp_pos+2 );
                } else if ( $tmp_pos = mb_strpos( $street, '地区', $streetlen ) ) {
                    $a4 = mb_substr( $street, 0, $tmp_pos+2 );
    ...
    ** 智能解析出省市区+街道地址
    */
    public static function parse( $a1, $a2, $a3, $a4 ) {
        require 'data/a4.php';
        require 'data/a3.php';
        require 'data/a2.php';
        require 'data/a1.php';
